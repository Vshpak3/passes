...
const PaymentSettings = ({ setSettingsNav }: PaymentSettingsProps) => {
  ...
  return (
    <>
      ...
      >
        <BackArrowIcon />
        <span className="text-[20px] font-[700]">Payment Settings</span>
      </div>

      <div className="my-8 flex gap-6">
        {isCreator && (
          <div className="flex h-[193px] w-[248px] flex-col justify-center gap-2 rounded-[20px] border border-passes-dark-200 bg-[#1B141D]/50 p-6">
            <span className="text-[14px] font-[400] opacity-90">
              Default Payout Method:
            </span>
            <div className="flex gap-6">
              <span className="text-[16px] font-[700]">Wells Fargo</span>
            ...
            <button
              ...
              onClick={() =>
                setSettingsNav(PaymentAndWalletSettingsEnum.MANAGE_BANK)
              }
            >
              <BankIcon width={25} height={25} />
              <span className="text-[16px] font-[500]">Manage Payout</span>
            </button>
          </div>
        )}
        <div className="flex h-[193px] w-[248px] flex-col justify-center gap-2 rounded-[20px] border border-passes-dark-200 bg-[#1B141D]/50 p-6">
          ...
          <span className="text-[16px] font-[700]">Anna DeGuzman Fargo</span>
          <div className="flex gap-6">
            <span className="text-[14px] font-[500] opacity-70">
              *******8920
            </span>
            <VisaIcon width={25} height={25} />
          </div>
          <button
            className="flex h-[44px] shrink-0 items-center justify-center gap-2 rounded-full border border-passes-primary-color bg-passes-primary-color px-2 text-white"
            onClick={() =>
          ...
          >
            <CardIcon width={25} height={25} className="mt-2" />
            <span className="text-[16px] font-[500]">Manage Payment</span>
          </button>
        </div>
        <div className="flex h-[193px] w-[248px] flex-col justify-center gap-6 rounded-[20px] border border-passes-dark-200 bg-[#1B141D]/50 p-6">
          <span className="text-[14px] font-[400] opacity-90">
            Default Wallet:
          </span>
          <div className="flex gap-6">
          ...
          <button
            className="flex h-[44px] shrink-0 items-center justify-center gap-2 rounded-full border border-passes-primary-color bg-passes-primary-color px-2 text-white"
            onClick={() =>
              setSettingsNav(PaymentAndWalletSettingsEnum.MANAGE_CARD)
            }
          >
            <WalletIcon width={25} height={25} />
            <span className="text-[16px] font-[500]">Manage Wallet</span>
          </button>
        </div>
      </div>
        ...
        <div className="flex flex-col rounded-[20px] border border-passes-dark-200 bg-[#1B141D]/50 p-4">
          <div className="mb-6 flex justify-between">
            ...
            <span
              onClick={() =>
                setSettingsNav(
                  PaymentAndWalletSettingsEnum.VIEW_ALL_TRANSACTIONS
                )
              }
              className="cursor-pointer text-[16px] font-[700] text-passes-pink-100"
            >
              View All
            </span>
          </div>
