...
export class Migration20220928000923 extends Migration {
  ...
  async up(): Promise<void> {
    ...
    this.addSql('alter table `post` add index `post_scheduled_at_index`(`scheduled_at`);');
    ...
    this.addSql('alter table `creator_earning` add unique `creator_earning_user_id_type_unique`(`user_id`, `type`);');
    ...
    this.addSql('alter table `default_wallet` add unique `default_wallet_user_id_unique`(`user_id`);');
    ...
    this.addSql('alter table `post_content` add constraint `post_content_post_id_foreign` foreign key (`post_id`) references `post` (`id`) on update cascade;');
    this.addSql('alter table `post_content` add constraint `post_content_content_id_foreign` foreign key (`content_id`) references `content` (`id`) on update cascade;');
    ...
    this.addSql('alter table `post_tip` add constraint `post_tip_payin_id_foreign` foreign key (`payin_id`) references `payin` (`id`) on update cascade;');
    ...
    this.addSql('alter table `circle_bank` add constraint `circle_bank_user_id_foreign` foreign key (`user_id`) references `users` (`id`) on update cascade;');
    ...
    this.addSql('alter table `payout` add constraint `payout_user_id_foreign` foreign key (`user_id`) references `users` (`id`) on update cascade;');
