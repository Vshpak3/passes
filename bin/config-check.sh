...
log 'Dev config keys:' > ${out}
dev=$(get_keys .env.dev)
echo "${dev}" | sed 's/^/  /' > ${out}

log 'Stage config keys:' > ${out}
stage=$(get_keys .env.stage)
echo "${stage}" | sed 's/^/  /' > ${out}

log 'Prod config keys:' > ${out}
prod=$(get_keys .env.prod)
echo "${prod}" | sed 's/^/  /' > ${out}

if [[ "${stage}" != "${dev}" ]] ; then
  log 'Stage config keys do not match dev config keys'
  exit 1
fi

if [[ "${prod}" != "${dev}" ]] ; then
  log 'Prod config keys do not match dev config keys'
  exit 1
fi
...
log 'All secrets listed in configs:' > ${out}
...
log 'All secrets listed in secret list:' > ${out}
